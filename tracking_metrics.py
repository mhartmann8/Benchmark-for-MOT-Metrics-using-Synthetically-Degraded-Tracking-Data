...
def calc_ctc_scores(result_dir, gt_dir):
    ...
    n_files = len([f for f in os.listdir(data_dir / data_set_id) if f.endswith("tif")])
    if n_files < 1000:
        n_digits = 3
    else:
        n_digits = 4
...
def calculate_trackeval_scores(gt_folder, res_folder, print_res):
    """
    Calculates the HOTA, CLEAR and Identity metrics using the trackeval library.
    Extracted from the run skripts of https://github.com/JonathonLuiten/TrackEval
    Defines the configuration, calculates the metrics and saves the result in the res_folder directory.
    Args:
        gt_folder: path to the gt folder
        res_folder: path to the tracking results
        print_res: parameter to toggle how much progress should be printed
    ...
    script_config = {
        ...
        "GT_FOLDER": gt_folder,  # os.path.join(code_path, 'data/gt/rob_mots'),
        "TRACKERS_FOLDER": res_folder,  # os.path.join(code_path, 'data/trackers/rob_mots'),,
    ...
    config["PRINT_RESULTS"] = print_res
    config["PRINT_CONFIG"] = False
    config["OUTPUT_DETAILED"] = False
    config["OUTPUT_EMPTY_CLASSES"] = False
    config["PRINT_ONLY_COMBINED"] = True
    config["DISPLAY_LESS_PROGRESS"] = True
...
def timing(f):
    """Wrapper to measure and print execution time of methods."""
